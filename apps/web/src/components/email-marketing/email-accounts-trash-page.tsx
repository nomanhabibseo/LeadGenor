...
export function EmailAccountsTrashPage() {
  ...
  const { data: rows = [], isLoading, refetch } = useQuery({
    queryKey: ["email-accounts-trash", userKey],
    queryFn: () => apiFetch<Row[]>("/email-marketing/accounts/trash", token),
    enabled: status === "authenticated" && !!token && !!userKey,
  });

  const restore = useMutation({
    mutationFn: (id: string) =>
      apiFetch(`/email-marketing/accounts/${id}/restore`, token, { method: "POST" }),
    onSuccess: () => {
      void qc.invalidateQueries({ queryKey: ["email-accounts-trash", userKey] });
  ...
  return (
    <div className="em-page mx-auto max-w-4xl space-y-6 px-2 pb-16 sm:px-4">
      <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Email accounts — trash</h1>
      <p className="text-sm text-slate-600 dark:text-slate-400">
        Restore an account to use it again from{" "}
        <Link className="font-semibold text-indigo-600 underline dark:text-indigo-400" href="/email-marketing/accounts">
          Email accounts
        </Link>
        .
      </p>
