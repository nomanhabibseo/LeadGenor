...
export function EmailCampaignsTrashPage() {
  ...
  const { data: rows = [], isLoading, refetch } = useQuery({
    queryKey: ["email-campaigns-trash", userKey],
    queryFn: () => apiFetch<Row[]>("/email-marketing/campaigns/trash", token),
    enabled: status === "authenticated" && !!token && !!userKey,
  });

  const restore = useMutation({
    mutationFn: (id: string) =>
      apiFetch(`/email-marketing/campaigns/${id}/restore`, token, { method: "POST" }),
    onSuccess: () => {
      void qc.invalidateQueries({ queryKey: ["email-campaigns-trash", userKey] });
  ...
  return (
    <div className="em-page mx-auto max-w-4xl space-y-6 px-2 pb-16 sm:px-4">
      <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Campaigns — trash</h1>
      <p className="text-sm text-slate-600 dark:text-slate-400">
        Restored campaigns return to{" "}
        <Link className="font-semibold text-indigo-600 underline dark:text-indigo-400" href="/email-marketing/campaigns">
          Campaigns
        </Link>
        .
      </p>
            ...
            >
              <div>
                ...
                <div className="text-xs text-slate-500">
                  List: {r.emailList.name} · {r.status}
                </div>
              </div>
              <button
                type="button"
